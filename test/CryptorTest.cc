...
TEST_F(CryptorTest, encode_and_decode_char_pointer) {
  int size = 20;
  char *str = new char[size];

  memset(str, static_cast<int>('a'), size);
  kcpuv::Cryptor::KcpuvProtocolEncode(1, str);

  EXPECT_EQ(static_cast<unsigned int>(str[0]), 1);
  EXPECT_EQ(static_cast<int>(str[2 + KCPUV_NONCE_LENGTH]),
            static_cast<int>('a'));

  int close = kcpuv::Cryptor::KcpuvProtocolDecode(str);
...
TEST_F(CryptorTest, init_and_clean_cryptor) {
  int key_len = 6;
  ...
  int rval = kcpuv::Cryptor::KcpuvCryptorInit(cryptor, key, key_len, salt);
  ...
  for (int i = 0; i < times; i += 1) {
    ...
    int len = content_len;
    char *ciphertext =
        reinterpret_cast<char *>(kcpuv::Cryptor::KcpuvCryptorEncrypt(
    ...
    char *deciphertext =
        reinterpret_cast<char *>(kcpuv::Cryptor::KcpuvCryptorDecrypt(
  ...
  kcpuv::Cryptor::KcpuvCryptorClean(cryptor);
