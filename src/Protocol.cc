 ...
 */
unsigned char *Protocol::kcpuv_cryptor_encrypt(kcpuv_cryptor *cryptor,
 ...
 */
unsigned char *Protocol::kcpuv_cryptor_decrypt(kcpuv_cryptor *cryptor,
...
int Protocol::kcpuv_cryptor_init(kcpuv_cryptor *cryptor, const char *key,
                                 int key_len, unsigned int salt[]) {
  cryptor->en = new EVP_CIPHER_CTX;
  cryptor->de = new EVP_CIPHER_CTX;
...
void Protocol::kcpuv_cryptor_clean(kcpuv_cryptor *cryptor) {
...
int Protocol::kcpuv_protocol_decode(const char *content) {
  // int ver = (*(const unsigned char *)content) & 0xff;
  int cmd = (*(const unsigned char *)(content + 1)) & 0xff;
  return cmd;
}

void Protocol::kcpuv_protocol_encode(int cmd, char *content) {
  // NOTE: reserved
